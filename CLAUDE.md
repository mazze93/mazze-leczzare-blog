...
- **`.claude/skills/` holds copies, not originals** — the skills published from `~/Projects/skills/claude-code-skills` are duplicated into this repo so a clone is self-contained. Two tracked copies in two repos drift silently (the run skill once named a macOS user that doesn't exist here, a framework major two behind, and a nine-route map for a fifty-page site). `check-docs-drift.sh` §10 compares them file-by-file when the source repo is present and says UNVERIFIED when it isn't. Edit the source, then copy here — never only one side.
